#!/usr/bin/env python3
"""Exploratory Data Analysis for Multilingual Toxic Comment Classification."""
...
def main():
    ...
    train_df["toxic"].value_counts().plot(kind="bar", ax=axes[0], color=["#2ecc71", "#e74c3c"])
    axes[0].set_title("Training Set: Label Distribution")
    axes[0].set_xticklabels(["Non-toxic", "Toxic"], rotation=0)
    axes[0].set_ylabel("Count")

    val_df.groupby("lang")["toxic"].mean().plot(kind="bar", ax=axes[1], color="#3498db")
    ...
    train_df["text_length"].clip(upper=2000).hist(bins=100, ax=ax, color="#3498db", alpha=0.7)
    ax.set_title("Training Set: Comment Length Distribution (clipped at 2000)")
    ax.set_xlabel("Character Length")
    ax.set_ylabel("Count")
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURES_DIR, "text_length_distribution.png"), dpi=150)
    plt.close()
    print(f"Saved: figures/text_length_distribution.png")

    # Plot 3: Text length by toxicity
    fig, ax = plt.subplots(figsize=(10, 5))
    for label, color, name in [(0, "#2ecc71", "Non-toxic"), (1, "#e74c3c", "Toxic")]:
        train_df[train_df["toxic"] == label]["text_length"].clip(upper=2000).hist(
